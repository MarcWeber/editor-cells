        ...
        for line in vim.current.buffer[:]:
            for w in line.split(' '):
                ...
                if (w == word_before_cursor):
                    continue
                line_diff = linenr - event['position'][1]
                if line_diff > 100:
                    certainity = 1
                else:
                    # words in lines nearby cursor are more important ..
                    certainity = 1 + abs(100.0 - line_diff) / 500
                if linenr > event['position'][1]:
                    # lines below cursor are less important than above cursor
                    certainity = sqrt(certainity)
                words[w] = {'word': w, 'certainity': certainity, 'description': w+' (py)'}
            linenr += 1

        completions = cells.util.match_by_type(words.values(), word_before_cursor, event['match_types'])
        print("replying comp", str(completions))
        self.reply_now(event, [{ 'column': event['position'][2] - len(word_before_cursor), 'completions' : completions }])
