...
class CompletionBasedOnFiles(Cell):
    ...
    def with_buffers(self, request):
        ...
        for result in request['results_good']:
            files_open_in_vim += [x['filename'] for x in result['buffers']]
    ...
    def with_files(self, request):
        ...
        def add(word, certainity):
            if word in words:
                words[word]['count'] += 1
                words[word]['certainity'] = max([words[word]['certainity'], certainity])
            else:
                words[word] = {'word': word, 'certainity': certainity, 'description': word+' (py)', 'count': 0}
        ...
        completions = words.values() # cells.util.match_by_type(words.values(), word_before_cursor, oe['event']['match_types'])
        ...
        for w in completions:
            w['certainity'] = w['certainity'] + math.atan(w['count'] / 10) # so result should should be somwhere between 1 and 2

        maxv = max([x['certainity'] for x in completions])
        for w in completions:
            w['certainity'] = w['certainity'] / maxv

        completions.sort(key = lambda a: -a['certainity']) # so results should be less than 1
        completions = completions[0:int(oe['event']['limit'])]
        self.async_reply(oe, [{ 'column': int(oe['event']['position'][2]) - len(word_before_cursor), 'completions' : completions }])
