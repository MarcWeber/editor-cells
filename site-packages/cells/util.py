...
def match_by_type(list, word, match_types):
    ...
    elif ('prefix_ignore_case' in match_types):
        regexes.append([re.compile('^\c'.word), 1])
    elif t == 'ycm':
        # order of chars .., match case, then without match
        regexes.append([re.compile('^\c'.word), 1])
        regexes.append([re.compile('^\c'.word), 1])
    elif len(word) <= 5 and t == 'camel_case_like':
        regexes.append([CamelCaseLikeMatching(word), 1])
    else:
        cells.Log('unkown match_type '.t)
