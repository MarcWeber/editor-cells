...
def process_event(event):
    debug_str("appending to process event %s " % str(event))
...
# # debug version
# loop_run = 0
# def process_debug(wait_for_ms):
#     globals()['loop_run'] += 1
#     print("==== loop run %s " % globals()['loop_run'])
#     tracer = trace.Trace(
#             countfuncs=1,
#             timing=True,
#             countcallers=1,
...
#         trace=1,
#         count=1)
#     x = tracer.runctx('process(wait_for_ms)', globals=globals(), locals=locals())
#     r = tracer.results()
