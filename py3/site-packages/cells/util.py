...
class FindAll(object):
    ...
    def __init__(self, word, regexes = None):
        super(FindAll, self).__init__()
        self.regexes = regexes

    def all(self, s):
        for re in self.regexes:
            yield from re.findall(s)
...
def match_by_type2(list, word):
  ...
  for c in list:
    r = f(c['word'])
    c['w'] = 0 if r == None else c.get('w', 0.9) * r
...
def import_thing(n):
    try:
        ...
        debug_str("%s while importing %s" % (error, module_and_class) )
        raise e
...
def by_path(path, dict):
    try:
        return reduce(operator.getitem, path, dict)
