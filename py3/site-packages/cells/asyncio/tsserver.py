...
class Tsserver(cells_a.Cell):
    ...
    def __init__(self, *args, **kwargs):
        ...
        def unexpected_message(r):
            ...
            if 'event' in r and r['event'] == 'syntaxDiag':
                self.errors['syntaxDiag']   = [r['body']] if type(r['body']) == type({}) else r['body']
            elif 'event' in r and r['event'] == 'semanticDiag':
    ...
    async def __completions(self, event):
        ...
        completions = [{'word': c['name']+( "(" if c['kind'] in ['function', 'method'] and not in_import else "") , 'w': 99999, 'menu': str(c), 'kind' : 'Tsserver' , 'info': c['kind']} for c in r['body']]
