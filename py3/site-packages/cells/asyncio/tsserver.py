...
class Tsserver(cells_a.Cell):
    ...
    async def __completions(self, event):
        ...
        completions = [{'word': c['name']+( "(" if c['kind'] in ['function','method'] else "") , 'w': 5, 'menu': str(c), 'kind' : 'Tsserver' , 'info': c['kind']} for c in r['body']]
    ...
    async def __definitions(self, event):
        ...
        [typeDefinitions, definitions] = await asyncio.gather(
        ...
        debug_str("RESULTs %s" % str( [ typeDefinitions, definitions ]))

        unique = [json.loads(s) for s in set([json.dumps(d) for d in typeDefinitions['body'] + definitions['body']])]
        ...
        def add_errors(c):
            for y in self.errors[c]:
                for x in y['diagnostics']:
                    ...
                    item = {'filename': y['file'], 'text': x['text'], 'col': x['start']['offset'], 'lnum': x['start']['line'], 'type' : categories[x['category']]}
                    debug_str('adding item %s' %(str(item)))
                    list.append(item)
