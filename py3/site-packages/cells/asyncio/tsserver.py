...
# [x] finish fix error 
# [ ] code ...
# [ ] what to do with suggestionDiag
# [ ] finish show type
# [ ] implement organize imports
# [ ] sort by cursor distance, finding definition is hard, but looking up distance the last 50 lines is faste.
#     eg p -> parseFloat is less like than local var or argument
...
extensions = re.compile('(.*\.(ts|tsx|js|jsx)$)|(^tsconfig.json$)')
...
class Tsserver(cells_a.Cell):
    ...
    def __init__(self, *args, **kwargs):
        ...
        def log_reader(s):
            # hack - for diag* replies the seq is in a body sub key for whatever reason
            if 'body' in s and 'request_seq' in s['body']:
                s['request_seq'] = s['body']['request_seq']
        ...
        def unexpected_message(r):
            ...
            # debug_str('>> tsserver got unexpected message %s '%(str(r)))
            handled = False
            for diag in ['syntaxDiag', 'semanticDiag', 'suggestionDiag']:
                if 'event' in r and r['event'] == diag:
                    self.errors[diag]   = [r['body']] if type(r['body']) == type({}) else r['body']
                    handled = True
            if not handled:
        ...
        # try showingfiles for current file

        asyncio.Task(self.__initial_errors_current_buf())
    ...
    async def __completions(self, event):
        ...
        if (not extensions.match(event['filepath'])):
            return []
        ...
        in_import = re.match('import', event['line_split_at_cursor'][0])
        completions = [{'word': c['name']+( "(" if c['kind'] in ['function', 'method'] and not in_import else "") , 'w': 1, 'menu': str(c), 'kind' : 'Tsserver' , 'info': c['kind']} for c in r['body']]
    ...
    async def message(self, message):
        await self.ask_editor_commands(commands = [{'show_message': message}])

    async def apply_changes(self, changes):
        await self.ask_editor_commands(commands = [{'apply_changes': changes}])

    # renaming / refactoring
    async def l_rename(self, event):
        if 'filepath' in event and 'pos' in filepath:
            cc = event
        else:
            cc = (await self.ask_editor_commands(commands = ['cursor_context']))[0][0]
        ...
        debug_str("cc is %s" % str(cc))
        r = await self.jsonrpc.call(command = "rename", type ='request', arguments = {'file': cc['filepath'], 'line': cc['position'][1], 'offset': cc['position'][2], 'findInComments' : event.get('findInComments'), 'findInString' : event.get('findInString'), 'replaceWith' : event.get('replaceWith')})
        # RenameResponseBody
        info = r['body']['info']
        debug_str("RESULTs %s" % str(r))
#  'success': True, 
# 'body': {'info': {'canRename': True, 'displayName': 'abc', 'fullDisplayName': 'abc', 'kind': 'let', 'kindModifiers': '', 'triggerSpan': {'start': {'line': 11, 'offset': 1}, 'end': {'line': 11, 'offset': 4}}}, 'locs': }}
# 
# 
# [{'file': '/pr/typescript/itemstorage/src/itemtypes.ts', 'locs': [
# {'start': {'line': 9, 'offset': 5}, 'end': {'line': 9, 'offset': 8}, 'contextStart': {'line': 9, 'offset': 1}, 'contextEnd': {'line': 9, 'offset': 16}}, 
# {'start': {'line': 11, 'offset': 1}, 'end': {'line': 11, 'offset': 4}},
# {'start': {'line': 12, 'offset': 1}, 'end': {'line': 12, 'offset': 4}}
# ]

        if 'canRename' in info:
            info = r['body']['info']
            to = (await self.ask({'type': 'input', 'question' : 'rename %s to :' % info['displayName']}))[0]

            def to_change(c):
                return 

            def to_changes(change_sets):
                change_sets.reverse()
                return [ {'replace_range_with': to, 'start': c['start'], 'end': c['end']} for c in change_sets]

            await self.apply_changes([ {'filepath': c['file'], 'changes': to_changes(c['locs'])} for c in r['body']['locs'] ])

            await this.message("rename failed: %s" % r['body']['localizedErrorMessage'])

                # if file in files
                #cfor c in change_for_file['locs'].reverse():



            # must implement or copy implementation from youcomplete me
        else:
            debug_str("rename failed %s" % str(r))
            await self.message("rename failed: %s" % r['body']['localizedErrorMessage'])
        ...
        def add_errors(c):
            for y in self.errors[c]:
                for x in y['diagnostics']:
                    ...
                    list.append({'filename': y['file'], 'text': "", 'col': x['start']['offset'], 'lnum': x['start']['line'], 'type' : categories[x['category']]})
                    if not elp.tss_any(text_lines[0], list):
                        for l in text_lines:
                            list.append({'text': l})
    ...
    async def l_showtype(self, event):
        # if (not extensions.match(event['filepath'])):
        #     return []
        # self.reset_errors()
        # asyncio.Task(self.__errors({'for_filepaths': [event['filepath']]}))
    ...
    async def l_fix_error(self, event):
        cc = (await self.ask_editor_commands(commands = ['cursor_context']))[0][0]
        if (not extensions.match(cc['filepath'])):
            return []
        if ('refresh_error_list' in event):
            debug_str("EXTENSION MATCHED")
            await asyncio.gather(*[self.filesstate.prepare_file(f) for f in [cc['filepath']]])
            debug_str("FILEPREPARED %s" % cc['filepath'])
            r = await self.jsonrpc.call(command = "geterr", arguments = {'delay': 0, 'files': [cc['filepath']]})
            debug_str("ERRORS %s")
            debug_str("ERRORS %s" % str(r))


        at_cursor = []
        line = int(cc['position'][1])
        col  = int(cc['position'][2])

        for diag in ['syntaxDiag', 'semanticDiag']:
            debug_str(str(self.errors[diag]))
            for x in self.errors[diag]:
                for e in x['diagnostics']:
                    debug_str("line %s col %s %s %s %s %s" % (line, col, e['start']['line'] <= line, e['start']['offset'] <= col, e['end']['line'] >= line, e['end']['offset'] >= col))
                    if e['start']['line'] <= line and e['start']['offset'] <= col and e['end']['line'] >= line and e['end']['offset'] >= col:
                        at_cursor.append(e)

        debug_str("ERRORS AT CURSOR %s" % str(at_cursor))

        x = await self.jsonrpc.call(command = "getCodeFixes", arguments = {
            'file': cc['filepath'],
            'startLine': line,
            'startOffset': col,
            'endLine': line,
            'endOffset': col+1,
            'errorCodes': [x['code'] for x in at_cursor]}
            )
        debug_str("FIXES %s" % str(x))


        # debugs FIXES {'seq': 0, 'type': 'response', 'command': 'getCodeFixes', 'request_seq': 14, 'success': True, 
        # 'body': [{'fixName': 'import', 'description': 'Import \'readFileSync\' from module "fs-extra"', 'changes': [{'fileName': '/pr/typescript/itemstorage/types.ts', 'textChanges': [{'start': {'line': 1, 'offset': 1}, 'end': {'line': 1, 'offset': 1}, 'newText': '  import { readFileSync } from "fs-extra";\n'}]}]}]}

        choices = [{'line': x['description'], 'return': x['changes'] } for x in x['body']]
        if (len(choices) == 0):
            return
        changes = (await self.ask({'type': 'choice', 'choices' : choices}))[0]
        debug_str("chosen %s" % str(changes))

        def convert_change(c):
            if 'newText' in c:
                return {'insert': c['newText'], 'start': c['start'] }
            else:
                self.message('unknown change %s ' % str(c))

        def convert_changes(changes):
            changes.reverse()
            return [convert_change(x) for x in changes]


        await self.apply_changes(
                [ {'filepath': c['fileName'], 'changes': convert_changes(c['textChanges']) } for c in changes]
        )


#     # code fixes
#     async def l_code_fixes(self, event):
#         if 'filepath' in event and 'pos' in filepath:
#             cc = event
#         else:
#             cc = (await self.ask_editor_commands(commands = ['cursor_context']))[0][0]

#         debug_str("errors %s" % str(self.errors))
...
#         #l_code_fixesr = await self.jsonrpc.call(command = "getCodeFixes", type ='request', arguments = {'errorCodes': })
#         #l_code_fixesinfo = r['body']['info']
#         #l_code_fixesdebug_str("RESULTs %s" % str(r))
