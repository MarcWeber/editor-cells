...
class Eclim(cells_a.Cell):
    ...
    async def __prepare_buf_php(self, event):
        ...
        word_before_cursor = re.match('[a-zA-Z-_]*$', lsac[0]).group(0)[1:]
        commands = []
        commands.append({'eval': 'eclim#project#util#GetProjectRelativeFilePath()'})
        commands.append({'set_current_line': "%s" % lsac[0][0:len(lsac[0]) - len(word_before_cursor)] + ' ' * len(word_before_cursor) + lsac[1]})
        commands.append('write_current_buffer_silently')
        commands.append({'set_current_line': "%s" % lsac[0]+lsac[1]})
        r = await self.ask_editor_commands(commands = commands)

        result = {
            'lsac': lsac,
            'project_path': r[0][0],
            'offset_minus': len(word_before_cursor.encode('utf-8')),
    ...
    async def __completions(self, event):
        ...
        return [{ 'column': column-1, 'completions' : completions }]
    ...
    async def __definitions(self, event):
        ...
        if file_extension == '.php':
            ...
            project_name = await self.ask_editor_commands(commands = [{'eval':'eclim#project#util#GetCurrentProjectName()'}])
