...
re_fusebox_1 = re.compile("Acorn error: (.*) \(([^:)]+):([^)]+)\)")
re_fusebox_2 = re.compile("File: (.*)")
def line_parser_fusebox(lps):
    match1 = re_fusebox_1.match(lps.line())
    match2 = re_fusebox_2.match(lps.line(+1))
    if (match1 and match2):
        lps.index +=2
        lps.append({'filename': match2.group(1), 'text': match1.group(1), 'lnum': match1.group(2), 'col': match1.group(3), 'type' : 'E'})
        return True
    return False

def tss_any(text, list):
    return tss_comp_types(text, list) or tss_is_not_assignable(text, list) or tss_is_missing_the_following_types(text, list)

re_tsc1 = re.compile("([^(]+)\(([0-9]+),([0-9]+)\): (.*)")
def line_parser_tsc(lps):
    match = re_tsc1.match(lps.line())
    if (match):
        text = match.group(4)
        lps.append({'filename': match.group(1), 'text': "", 'lnum': match.group(2), 'col': match.group(3), 'type' : 'E'})
        if not tss_any(text, lps.result_list):
            lps.append({'text': text})
        lps.index+=1
        return True
    return False
...
# Type '{ event_file_found: undefined[]; file_as_json_a_h: file_as_json_a_h; file_as_string_a_h: file_as_string_a_h; file_analysed_a_h: file_analysed_a_h; resolveImplementation: ResolveImplementation; }' is missing the following properties from type 'GCTX': exists, fileType, log, throwError
re_tss_is_missing_the_following_types = re.compile('(Type \'.*\') (is missing the following properties from type \'.*\':) (.*)')
def tss_is_missing_the_following_types(s, list):
    match = re_tss_is_missing_the_following_types.match(s)
    if (match):
        for g in [1,2,3]:
            list.append({'text': match.group(g)})
        return True
...
def line_parser_webpack(lps):
    ...
    if (match):
        ...
        if tss_any(message, lps.result_list):
            return 
        else:
