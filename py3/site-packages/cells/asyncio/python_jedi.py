...
class JediCompletion(cells_a.Cell):
    ...
    async def __completions(self, event):
        ...
        def info(c):
            ...
            return "params: %s\nmodule_path: %s:%s\n%s" % (params_to_str(c), c.module_path, c.line, c.doc)
        ...
        completions = [{'word': c.name, 'w': 1.1, 'menu': "from %s" % c.module_name, 'kind' : c.type, 'description': c.get('doc',''), 'info': info(c) } for c in completions if re_match.match(c.name)]
    ...
    async def __definitions(self, event):
        ...
        return [{'filepath': event['filepath'] if c.module_path is None else c.module_path, 'line': c.line, 'column': 1+c.column, 'title': c.name, 'text': c.get('doc',''), 'kind': 'JediCompletion'} for c in definitions]
