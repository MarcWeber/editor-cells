...
class JediCompletion(cells_a.Cell):
    ...
    async def __script(self, event, column = None, line = None):
        lines = await self.ask_editor_commands(commands = [{'buffer_lines':1}])
        lines = lines[0][0]
    ...
    async def __completions(self, event):
        ...
        completions = [{'word': c.name, 'w': 1.1, 'menu': "from %s" % c.module_name, 'kind' : c.type, 'description': c.doc, 'info': info(c) } for c in completions if re_match.match(c.name)]
