...
class JediCompletion(cells_a.Cell):
    ...
    async def __completions(self, event):
        ...
        def info(c):
            ...
            return "params: %s\nmodule_path: %s:%s\n%s" % (params, c.module_path, c.line, c.doc)
        ...
        completions = [{'word': c.name + params_to_str(c), 'w': 1.1, 'menu': "from %s" % c.module_name, 'kind' : c.type, 'description': c.doc, 'info': info(c) } for c in completions if re_match.match(c.name)]
