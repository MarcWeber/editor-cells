...
class BufferChanges:
    ...
    def __init__(self, bufid, cell):
        self.cell = cell
        self.bufid = bufid
        self.lines = None

    async def change_message(cursor_line):
        if self.lines == None:
            return await all_lines()
        else:
            r = await self.cell.ask_editor_commands(commands = [])
    ...
    async def process_incoming_reply(self):
        try:
            ...
            if m:
                ...
                debug_str("got content as json %s" % str(r))
                if r['id'] in self.waiting:
                    self.waiting.pop(r['id']).set_result(r)
                else:
                    # showMessage case or such?
                    debug_str("got unexpected message %s" % r)
                    cells_a.emit({'type': 'editor_commands', 'commands': [{'show_message': "unhandled server protocol server message: %s " % str(r)}]})
            else:
                raise "Expected Content-Length-Line, but got %s " % l1
