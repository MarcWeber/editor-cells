    ...
    async def receive_msg(self):
        ...
        while True:
            ...
            if m:
                ...
                raise Exception("unexpected line: %s" % str(l_decoded))
...
class chan_cmd_node_ipc():
    ...
    async def create(**kwargs):
        ...
        if pid == 0:
            ...
            reader = asyncio.StreamReader(loop=loop, limit = 1024 *1024 *5)
...
class JsonRPC():
    ...
    def __init__(self, connection_properties, init_calls = [], wait_for_server_messages = True, log_writer = None, log_reader = None, id_key = "id", reply_id_key = "id", unexpected_message = None):
        self.id = 1

        self.id_key = id_key
        self.reply_id_key = reply_id_key
        ...
        if unexpected_message == None:
            ...
            def __unexpected_message(r):
                debug_str("unexpected message %s" % str(r))
            self.unexpected_message = __unexpected_message
        else:
    ...
    async def send_with_seq(self, **kwargs):
        """ same as call but no id and don't wait for a result"""
        if self.chan == None:
            await self.reconnect()
    ...
    async def process_incoming_reply(self):
        try:
            ...
            if self.reply_id_key in r and r[self.reply_id_key] in self.waiting:
                self.waiting.pop(r[self.reply_id_key]).set_result(r)
            else:
                self.unexpected_message(r)
...
#      print("sent %s" % str(s))
#  
#  jsonrpc = JsonRPC(connection_properties = {'cmd': 'tee /tmp/in |node --inspect-brk --inspect /pr/tmp/crane2/client/server/server.js --stdio |tee /tmp/out', 'id': 'PHPCompletion'}, init_calls = [ init_call(), buildFromFiles_call() ], log_writer = log_writer, log_reader = log_reader)
#  
#  loop = asyncio.get_event_loop()
#  
#  async def test():
