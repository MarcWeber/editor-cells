...
class WatchCmdErrors(cells_a.Cell):
    ...
    async def l_quickfix_list(self, event):
        debug_str('WatchCmdErrors in l_quickfix_list')
    ...
    async def forever_read(self, cmd):
        ...
        self.task = None
        debug_str('WatchCmdErrors new_run regex %s ' % self.new_run_regex)
        new_run = re.compile(self.new_run_regex)
        while True:
            l = await self.reader.readline()
            debug_str('WatchCmdErrors got line %s' % l)
            l = l.decode('utf-8')
            if new_run.match(l):
                debug_str('WatchCmdErrors new_run detected')
                self.reset_lines()
            self.lines.append(l)
            if self.task != None:
                self.task.cancel()
    ...
    def send_stdin(self, data):
        # debug_str("WatchCmdErrors sending data to stdin: %s" % str(data))
