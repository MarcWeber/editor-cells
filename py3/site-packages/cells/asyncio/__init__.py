    ...
    async def emit(self, event, py_cells):
        ...
        for cell in [cell for cell in py_cells if hasattr(cell, l_name)]:
            ...
            if callable(listener):
                debug_str("AWAITING %s" % str(event))
...
class Cell(object):
    ...
    #     cell_collection.asyncio_loop.call_soon(*args, **kwargs)
    ...
    async def forward_event(self, event, override_keys):
        e = copy.copy(event)
        cell_collection.clean_event(e)
        e.update(override_keys)
        await emit(e)
        for wait_for_id in e['wait_for']:
    ...
    async def ask(self, event, request = None, results_only = True):
        ...
        # self.ask({'type': name, 'key': 'value'}) can be written as self.ask_name(key = value)
        results = []
        errors = []
    ...
    def show_message(text):
        asyncio.Task(cells_a.emit({'type': 'commands', 'commands': [{'show_message': text}], 'sender': self.id}))


    def show_error(text):
        asyncio.Task(cells_a.emit({'type': 'commands', 'commands': [{'show_error': text}]  , 'sender': self.id}))
  
    ### native buffer implementation for speed if available
    async def ask_editor_commands(self, commands = []):


        if hasattr(self, 'vim'):

            def buffer_by_id(bufid):
                cmd = 'bufnr(%s)' % json.dumps(bufid)
                return self.vim.eval(cmd) if type(bufid) == type('') else bufid

            # if we have vim module avoid message passing
            results = []
            for command in commands:
                if command == "write_current_buffer":
                    self.vim.command('update')
                    results.append(results)
                elif command == "write_current_buffer_silently":
                    self.vim.command('silent noautocmd update')
                elif command == 'buffers':
                    # TODO implement natively
                    r = await self.ask({'type': 'editor_commands', 'commands' : ['buffers']})
                    debug_str(str(r))
                    results.append(r[0][0])
                elif command == 'cursor_context':
                    results.append(self.vim.eval("cells#util#CursorContext({'position': getpos('.')})"))
                elif type(command) == type({}):
                    if 'apply_changes' in command:
                        return await self.forward_event({'commands': commands}, {'type': 'editor_commands'})
                    if 'show_message' in command:
                        cells.util.to_vim(command['show_message'])
                        self.vim.command('echom g:to_vim')
                        results.append(None)
                    if 'open_filepath' in command:
                        vim.eval('e %', str(command['open_filepath']))
                        results.append(vim.eval('bufnr(%)', str(command['open_filepath'])))
                    if 'buf_id_of_path' in command:
                        results.append(vim.eval('bufnr(%)', str(command['buf_id_of_path'])))
                    if 'show_error' in command:
                        cells.util.to_vim(command['show_error'])
                        self.vim.command('echoe g:to_vim')
                        results.append(None)
                    if 'eval' in command:
                        results.append(self.vim.eval(command['eval']))
                    elif "set_current_line" in command:
                        cells.util.to_vim(command['set_current_line'])
                        self.vim.command('call setline(".", g:to_vim)')
                        results.append("done")
                    elif 'save_as_tmp' in command:
                        cells.util.to_vim(command['save_as_tmp'])
                        self.vim.command("let ei=&ei| set ei=all| exec 'silent! w! '.fnameescape(g:to_vim) | exec 'set ei='.ei")
                        results.append("done")
                    elif 'lines_of_buf_id' in command:
                        bufid = buffer_by_id(command['lines_of_buf_id'])
                        try:
                            if bufid == '%':
                                buffer = self.vim.current.buffer
                            else:
                                buffer = self.vim.buffers[int(bufid)]
                            lines = buffer[int(command.get('from_line', 0)):cells.util.int_or_none(command.get('to_line', None))]
                        except Exception as e:
                            # maybe buf was closed ..
                            lines = []
            ...
            # module running outside of Vim, send commands to be executed to Vim
            x = await self.forward_event({'commands': commands}, {'type': 'editor_commands'})
            return x
...
class CellPy(Cell):
    ...
    async def l_cell_new_by_name(self, event):
        if event.get('network', cell_collection.prefix) != cell_collection.prefix:
            debug_str("ignoring cell_new_by_name event %s " % str(event))
            return
        try:
            thing = cells.util.import_thing(event['name'])
            thing(*event.get('args', []), **event.get('kwargs', {}))
        except Exception as e:
            debug_str('importing %s failed' % event['name'])
            raise e
        # TODO reply
...
async def emit(event):
    ...
    debug_str("emitting to emit_selector in __init %s " % str(event) )
