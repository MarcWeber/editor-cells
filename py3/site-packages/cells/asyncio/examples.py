...
class EditorImplementationUsingVimModuleIfPossible(cells.Cell):
    # TODO move into utils file or such
    # this is actually very useful

    # usage EditorImplementationUsingVimModuleIfPossible() -> create singleton
    # (or override it) because id is already set
    # then isntead of using l_editor_buffers use l_py_editor_buffers

    def __init__(self, *args, **kwargs):
        kwargs['id'] = cells.cell_collection.singleton_id('EditorImplementationUsingVimModuleIfPossible')
        super(EditorImplementationUsingVimModuleIfPossible, self).__init__(*args, **kwargs)
        try:
            self.vim = __import__('vim')
        except Exception as e:
            1/1

    """editor interface, can only be used when Python runs inside Vim. Otherweise use VimL implementation"""
    ...
    async def l_py_editor_buffers(self, event):
        ...
        if hasattr(self, 'vim'):
            ...
            # ask vim via event
            await self.forward_event(event, {'type': 'l_editor_buffers'})

    async def l_py_editor_buffer_lines(self, event):
        raise Exception('TODO')
        if not 'bufid' in event:
            event['reply_now'](self.id, vim.current[event.get('from_line', 0):event.get('to_line', None)])
        else:
            raise Exception('TODO')


class CompletionBasedOnFiles(cells.Cell):
    ...
    async def completions(self, oe):
        ...
        [files_open_in_vim, project_files, lines_current_buffer] = await asyncio.gather(
          self.ask({'type': 'editor_buffers'}),
          self.ask({'type': 'project_files'}),
          self.ask({'type': 'editor_buffer_lines'})
        )

        files_open_in_vim = sum([[x['filename'] for x in result['buffers']] for result in files_open_in_vim], [])
        ...
        find_all = util.CompletionWordsFromStr(word_before_cursor, oe['event']['match_types'])
        ...
        # TODO get lines of buffers not having a filename (which are not on disk)
        # or generalle get text from editor - maybe with caching and modify timestmap?
        for file in list(files_open_in_vim):
            ...
            if len(words) > 100000:
                break
        completions = list(words.values()) # util.match_by_type(words.values(), word_before_cursor, oe['event']['match_types'])
...
class Quickfix(cells.Cell):

    def __init__(self):
        super(Quickfix, self).__init__()
        Task(cells.emit({'type': 'quickfix_list_available', 'sender': self.id}))
