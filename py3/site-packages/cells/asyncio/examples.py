...
class CompletionBasedOnFiles(cells_a.Cell):
    ...
    async def completions(self, oe):
        ...
        [ret, project_files] = await asyncio.gather(
          self.ask_editor_commands(commands = ['buffers', {'lines_of_buf_id':'%'}]),
        ...
        debug_str("THERE%s"%str(results))
        certainity = 1
        for (bufid, lines) in zip(bufids, results[0]):
            debug_str(str(files_open_in_vim))
            last_line = int(files_open_in_vim['buffers'][bufid]['last_cursor_pos'][1]) if 'last_cursor_pos' in files_open_in_vim['buffers'][bufid] else -500
            for (line_0, line) in enumerate(lines):
                line_dist = abs(line_0 - last_line)
                f_distance = 1 if line_dist == 0 else (1.0 / line_dist) / 5 + 0.8
                for word in find_all.all(line):
                    # words not nearby last cursor position or buffers having used earlier get less weight
                    add(word, certainity * f_distance * (10 if re_last_upper != None and re_last_upper.match(word) else 1))
            certainity *= 0.8
